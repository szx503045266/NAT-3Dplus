...
from natformer.natten.extras import get_gflops, get_mparams
...
def evaluate(test_loader, model, criterion, optimizer, epoch, args, cfg, tf_writer):
	...
	with torch.no_grad():
		for cur_iter, (frames, labels, video_idx, _) in enumerate(test_loader):
			...
			if 'nat_' in cfg.MODEL.BACKBONE:
				print("FLOPs: ", get_gflops(model, clip))
				print("Params: ", get_mparams(model))
			else:
