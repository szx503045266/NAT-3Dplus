...
from natformer.natten import NeighborhoodAttention_tct_roll_split
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7,
        ...
        self.attn = NeighborhoodAttention_tct_roll_split(
...
def nat_mini(pretrained=True, **kwargs):
    model = NAT(depths=[3, 4, 6, 5], num_heads=[4, 4, 8, 16], embed_dim=64, mlp_ratio=3,
    ...
    if pretrained:
        ...
        for key in checkpoint.keys():
            if 'levels.0' in key and 'rpb' in key:
                checkpoint[key] = torch.concat((checkpoint[key],checkpoint[key]), 0)
