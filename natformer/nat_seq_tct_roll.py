...
from natformer.natten import NeighborhoodAttention_seq_tct_roll
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_scale=None, n_seg=0):
        super().__init__()
        self.dim = dim
        ...
        self.attn = NeighborhoodAttention_seq_tct_roll(
    ...
    def __init__(self, dim, depth, num_heads, kernel_size, downsample=True,
        ...
        self.blocks = nn.ModuleList([
            NATLayer(dim=dim,
                     ...
                     norm_layer=norm_layer, layer_scale=layer_scale, n_seg=self.n_seg)
...
def nat_mini(pretrained=True, **kwargs):
    ...
    # print(model.get_parameter('levels.3.blocks.2.attn.qkv_f.weight'))
    # print(model.get_parameter('levels.3.blocks.2.attn.qkv_s.weight'))
    # print(model.get_parameter('levels.2.blocks.3.attn.rpb_f'))
    if pretrained:
        ...
        for key in checkpoint.keys():
            if 'qkv' in key:
                key1 = key.replace('qkv','qkv_f')
                key2 = key.replace('qkv','qkv_s')
                key3 = key.replace('qkv','qkv_b')
                new_params[key1] = new_params[key2] = new_params[key3] = checkpoint[key]
            elif 'rpb' in key:
                key1 = key.replace('rpb','rpb_f')
                key2 = key.replace('rpb','rpb_s')
                key3 = key.replace('rpb','rpb_b')
                new_params[key1] = new_params[key2] = new_params[key3] = checkpoint[key]
        for key in new_params.keys():
            checkpoint[key] = new_params[key]
