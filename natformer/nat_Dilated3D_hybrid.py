...
from natformer.natten import NeighborhoodAttention_3D, NeighborhoodAttention_Dilated3D_hybrid
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7,
        ...
        if dilated_step > 0:
            self.attn = NeighborhoodAttention_Dilated3D_hybrid(
            ...
            self.attn = NeighborhoodAttention_3D(
                dim, kernel_size=kernel_size, num_heads=num_heads,
...
def nat_mini(pretrained=True, **kwargs):
    model = NAT(depths=[3, 4, 6, 5], num_heads=[2, 4, 8, 16], embed_dim=64, mlp_ratio=3,
                 drop_path_rate=0.2, kernel_size=7, dilated_step=[3,3,1,0], **kwargs)
    if pretrained:
