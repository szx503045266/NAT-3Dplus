...
from natformer.natten import NeighborhoodAttention_3D_GlobalT_RmDup, NeighborhoodAttention_3D_GlobalT_CondenseContext_RmDup
...
model_urls = {
    ...
    "nat_large_1k": "https://shi-labs.com/projects/dinat/checkpoints/imagenet1k/dinat_large_in22k_in1k_224.pth",
    "nat_large_21k": "https://shi-labs.com/projects/dinat/checkpoints/imagenet22k/dinat_large_in22k_224.pth",
}
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_scale=None, n_seg=0, down_spat=1):
        super().__init__()
        self.dim = dim
        ...
        if down_spat > 0:  ###1
            self.attn = NeighborhoodAttention_3D_GlobalT_CondenseContext_RmDup(
                dim, kernel_size=kernel_size, num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_seg=self.n_seg, down_spat=down_spat)
        else:
            self.attn = NeighborhoodAttention_3D_GlobalT_RmDup(
                dim, kernel_size=kernel_size, num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_seg=self.n_seg)
...
class NATBlock(nn.Module):
    def __init__(self, dim, depth, num_heads, kernel_size, downsample=True,
                 ...
                 layer_scale=None, n_seg=0, down_spat=1):
        super().__init__()
        self.dim = dim
        self.depth = depth
        self.n_seg = n_seg
        ...
        self.blocks = nn.ModuleList([
            NATLayer(dim=dim,
                     ...
                     norm_layer=norm_layer, layer_scale=layer_scale, n_seg=self.n_seg, down_spat=down_spat)
...
class NAT(nn.Module):
    def __init__(self,
        ...
        self.mlp_ratio = mlp_ratio
        self.n_seg = n_seg
        print("Down Spat Setting for Global Temporal {}".format(down_spat))
...
@register_model
def nat_tiny(pretrained=True, **kwargs):
...
@register_model
def nat_small(pretrained=True, **kwargs):
...
@register_model
def nat_base(pretrained=True, **kwargs):
...
@register_model
def nat_base_320(pretrained=True, **kwargs):
    model = NAT(depths=[3, 4, 18, 5], num_heads=[4, 8, 16, 32], embed_dim=128, mlp_ratio=2,
                drop_path_rate=0.5, layer_scale=1e-5, kernel_size=11, **kwargs)
    if pretrained:
        url = model_urls['nat_base_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        checkpoint.pop('head.weight')
        ...
        for k in checkpoint.keys():
            if ".rpb" in k:
                #print(checkpoint[k].shape[0])
                checkpoint[k] = torch.nn.functional.interpolate(
                    checkpoint[k].unsqueeze(0), size=(21, 21),
                    mode='bilinear'
                    ).squeeze(0)
        model.load_state_dict(checkpoint, strict=False)
    return model
...
@register_model
def nat_large(pretrained=True, **kwargs):
    model = NAT(depths=[3, 4, 18, 5], num_heads=[6, 12, 24, 48], embed_dim=192, mlp_ratio=2,
                drop_path_rate=0.5, layer_scale=1e-5, kernel_size=7, **kwargs)
    if pretrained:
        url = model_urls['nat_large_1k']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu")
        checkpoint.pop('head.weight')
        checkpoint.pop('head.bias')
        model.load_state_dict(checkpoint, strict=False)
