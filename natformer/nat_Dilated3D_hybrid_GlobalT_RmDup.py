...
from natformer.natten import NeighborhoodAttention_3D_GlobalT_RmDup, NeighborhoodAttention_Dilated3D_hybrid_GlobalT_RmDup
...
class NATLayer(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=7,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, layer_scale=None, n_seg=0, dilated_step=1):
        super().__init__()
        self.dim = dim
        ...
        if dilated_step > 0:
            self.attn = NeighborhoodAttention_Dilated3D_hybrid_GlobalT_RmDup(
                dim, kernel_size=kernel_size, num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, n_seg=self.n_seg, dilated_step=dilated_step)
        else:
...
class NATBlock(nn.Module):
    def __init__(self, dim, depth, num_heads, kernel_size, downsample=True,
                 ...
                 layer_scale=None, n_seg=0, dilated_step=1):
        super().__init__()
        self.dim = dim
        self.depth = depth
        self.n_seg = n_seg
        ...
        self.blocks = nn.ModuleList([
            NATLayer(dim=dim,
                     ...
                     norm_layer=norm_layer, layer_scale=layer_scale, n_seg=self.n_seg, dilated_step=dilated_step)
...
def nat_mini(pretrained=True, **kwargs):
    model = NAT(depths=[3, 4, 6, 5], num_heads=[2, 4, 8, 16], embed_dim=64, mlp_ratio=3,
                 drop_path_rate=0.2, kernel_size=7, dilated_step=[7,3,1,0], **kwargs)
    if pretrained:
